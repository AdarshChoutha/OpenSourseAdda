...
const { authorizeToken_fetch } = require('./controllers/middleware_function');
...
app.get('/', (req, res) => {
    dbConnection.query("USE osa_database", function (error, result) {
        if (error) {
            console.log("/: Cannot connect to osa_database");
...
app.post('/login', [
    ...
    if (!errors.isEmpty()) return res.json(errors.array());
        ...
        dbConnection.query("USE osa_database", function (error, result) {
            if (error) {
                console.log("/login: Cannot connect to osa_database");
                return res.json([{ "msg": "Something Went Wrong" }]);
            }
                        ...
                        dbConnection.query("SELECT user_password FROM user_accounts WHERE email_id = ?", [req.body.login_email], function (error, result) {
                            ...
                            bcrypt.compare(req.body.login_password, result[0].user_password, function (error, isMatch) {
                                ...
                                if (isMatch) {
                                    ...
                                    res.cookie("OSAJWT", accessToken, {
                                        expires: new Date(Date.now() + '4d'),
                                        httpOnly: true
                                    });
...
app.post('/check_registration_email_entered', [
    ...
    if (!errors.isEmpty()) return res.json(errors.array());
        ...
        dbConnection.query("USE osa_database", function (error, result) {
            if (error) {
                console.log("/check_registration_email_entered: Cannot connect to osa_database");
...
app.post('/register', [
    ...
    if (!errors.isEmpty()) return res.json(errors.array());
        ...
        dbConnection.query("USE osa_database", function (error, result) {
            if (error) {
                console.log("/register: Cannot connect to osa_database");
                return res.json([{ "msg": "Something Went Wrong" }]);
            }
