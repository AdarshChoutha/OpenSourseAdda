...
function authorizeToken_fetch(req, res, next) {
    if (req.cookies.OSAJWT) {
        authorizationToken = req.cookies.OSAJWT;
        if (authorizationToken == null) return res.json([{ "msg": "Unauthorized" }]);
        dbConnection.query("USE osa_database", function (error, result) {
            if (error) {
                console.log("F_authorizeToken: Cannot connect to osa_database");
                return res.json([{ "msg": "Something Went Wrong" }]);
            }
            jwt.verify(authorizationToken, process.env.ACCESS_TOKEN_SECRET, (error, user) => {
...
function enrollStatus(req, res, next) {
    ...
    dbConnection.query("USE osa_database", function (error, result) {
        if (error) {
            console.log("F_enrollStatus: Cannot connect to osa_database");
            return res.json([{ "msg": "Something Went Wrong" }]);
        }
